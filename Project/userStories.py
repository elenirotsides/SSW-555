...
def bigamy(families_dict, individual_id, marriage_date):
    """Marriage should not occur during marriage to another spouse. Returns True if marriage does occur during
    marriage to another spouse. This function should be called before a marriage is added to the families dict."""

    for famID, family in families_dict.items():
        if (individual_id == family["Husband ID"] or individual_id == family["Wife ID"]) and (
            (family["Divorced"] != "NA" and marriage_date < family["Divorced"]) or
